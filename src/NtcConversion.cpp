...
bool ConvertResistanceToTemperatureSteinhartHart(float resistance_ohms,
                                                float a,
                                                float b,
                                                float c,
                                                float* temperature_celsius) noexcept {
...
bool ConvertTemperatureToResistanceSteinhartHart(float temperature_celsius,
                                                float a,
                                                float b,
                                                float c,
                                                float* resistance_ohms) noexcept {
...
bool ConvertResistanceToTemperatureDefault(float resistance_ohms,
                                         float* temperature_celsius) noexcept {
    return ConvertResistanceToTemperatureSteinhartHart(resistance_ohms,
                                                      NTC_STEINHART_HART_A,
                                                      NTC_STEINHART_HART_B,
                                                      NTC_STEINHART_HART_C,
                                                      temperature_celsius);
}

bool ConvertTemperatureToResistanceDefault(float temperature_celsius,
                                         float* resistance_ohms) noexcept {
    return ConvertTemperatureToResistanceSteinhartHart(temperature_celsius,
                                                      NTC_STEINHART_HART_A,
                                                      NTC_STEINHART_HART_B,
                                                      NTC_STEINHART_HART_C,
                                                      resistance_ohms);
...
bool CalculateThermistorVoltage(float resistance_ohms,
                               float reference_voltage,
                               float series_resistance,
...
bool ValidateTemperature(float temperature_celsius, float min_temperature, float max_temperature) noexcept {
...
bool CalculateSteinhartHartCoefficients(float t1, float r1,
                                       float t2, float r2,
                                       float t3, float r3,
    ...
    if (!ValidateTemperature(t1, -273.15f, 1000.0f) || 
        !ValidateTemperature(t2, -273.15f, 1000.0f) || 
        !ValidateTemperature(t3, -273.15f, 1000.0f)) {
        return false;
    }
    
    if (!ValidateResistance(r1, 0.1f, 1000000.0f) || 
        !ValidateResistance(r2, 0.1f, 1000000.0f) || 
...
bool CalculateTemperatureAccuracy(float resistance_ohms,
    ...
    *accuracy_celsius = std::sqrt(temp_error_resistance * temp_error_resistance + 
...
bool CalculateOptimalSeriesResistance(float thermistor_resistance_at_25c,
    ...
    float r_min, r_max;
    if (!ConvertTemperatureToResistanceBeta(min_temperature, thermistor_resistance_at_25c, 3435.0f, &r_min) ||
        !ConvertTemperatureToResistanceBeta(max_temperature, thermistor_resistance_at_25c, 3435.0f, &r_max)) {
        return false;
    }
