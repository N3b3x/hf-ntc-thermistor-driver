...
ntc_err_t NtcThermistor::ReadTemperatureCelsius(float* temperature_celsius) noexcept {
    ...
    *temperature_celsius = raw_temperature + config_.calibration_offset;
    ...
    if (!ValidateTemperature(*temperature_celsius, config_.min_temperature, config_.max_temperature)) {
...
ntc_err_t NtcThermistor::GetRawAdcValue(uint32_t* adc_value) noexcept {
    ...
    return static_cast<ntc_err_t>(adc_interface_->ReadChannelCount(config_.adc_channel, *adc_value, 
                                                                  config_.sample_count, config_.sample_delay_ms));
}
...
ntc_err_t NtcThermistor::SetSamplingParameters(uint32_t sample_count, uint32_t sample_delay_ms) noexcept {
    if (sample_count == 0) {
        return NTC_ERR_INVALID_PARAMETER;
    }
...
const char* NtcThermistor::GetErrorString(ntc_err_t error) noexcept {
    switch (error) {
        case NTC_SUCCESS: return "Success";
        case NTC_ERR_FAILURE: return "General failure";
        case NTC_ERR_NOT_INITIALIZED: return "Not initialized";
        case NTC_ERR_ALREADY_INITIALIZED: return "Already initialized";
        case NTC_ERR_INVALID_PARAMETER: return "Invalid parameter";
        case NTC_ERR_NULL_POINTER: return "Null pointer";
        case NTC_ERR_OUT_OF_MEMORY: return "Out of memory";
        case NTC_ERR_ADC_READ_FAILED: return "ADC read failed";
        case NTC_ERR_INVALID_RESISTANCE: return "Invalid resistance value";
        case NTC_ERR_TEMPERATURE_OUT_OF_RANGE: return "Temperature out of range";
        case NTC_ERR_LOOKUP_TABLE_ERROR: return "Lookup table error";
        case NTC_ERR_CONVERSION_FAILED: return "Temperature conversion failed";
        case NTC_ERR_CALIBRATION_FAILED: return "Calibration failed";
        case NTC_ERR_UNSUPPORTED_OPERATION: return "Operation not supported";
        case NTC_ERR_TIMEOUT: return "Operation timeout";
        case NTC_ERR_HARDWARE_FAULT: return "Hardware fault";
...
const char* NtcThermistor::GetTypeString(ntc_type_t type) noexcept {
    switch (type) {
        case NTC_TYPE_UNKNOWN: return "Unknown";
        case NTC_TYPE_NTCG163JFT103FT1S: return "NTCG163JFT103FT1S";
        case NTC_TYPE_NTCG164JF103FT1S: return "NTCG164JF103FT1S";
        case NTC_TYPE_NTCG163JF103FT1S: return "NTCG163JF103FT1S";
        case NTC_TYPE_CUSTOM: return "Custom";
...
ntc_err_t NtcThermistor::ReadAdcVoltage(float* voltage_volts) noexcept {
    ...
    return static_cast<ntc_err_t>(adc_interface_->ReadChannelV(config_.adc_channel, *voltage_volts, 
                                                              config_.sample_count, config_.sample_delay_ms));
}
...
ntc_err_t NtcThermistor::ConvertResistanceToTemperature(float resistance_ohms, float* temperature_celsius) noexcept {
    if (temperature_celsius == nullptr) {
        return NTC_ERR_NULL_POINTER;
    }
    ...
    switch (config_.conversion_method) {
        case NTC_CONVERSION_LOOKUP_TABLE: {
            ...
            if (table != nullptr && FindTemperatureFromLookupTable(table, resistance_ohms, temperature_celsius)) {
                return NTC_SUCCESS;
            }
